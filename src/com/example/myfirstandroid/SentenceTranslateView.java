...
public class SentenceTranslateView extends Activity {
	TableLayout layout;
...
//		initLayout();
	...
	private void initSpinnerPosition(){
		int lessonId = CommonTool.getCurLesson(this,"sentenceLessonId");
...
//		sp.set
	...
	private class NextOnClick implements OnClickListener{
		...
		public void onClick(View arg0) {
			...
			if(lesId==getLesson().get(getLesson().size()-1).getClassNo()){   //如果当前已经是最后一课了，则提示
				Toast.makeText(SentenceTranslateView.this, "当前已经是最后一课！",Toast.LENGTH_LONG).show();
			}else{
	...
	private class LastOnClick implements OnClickListener{
		...
		public void onClick(View arg0) {
			...
			if (lesId == getLesson().get(0).getClassNo()) {   //如果当前已经是第一课了，则提示
				Toast.makeText(SentenceTranslateView.this, "当前已经是第一课！",Toast.LENGTH_LONG).show();
			} else {
				sp.setSelection(sp.getSelectedItemPosition()-1);
			}
		}
	...
	private class OnClassSelect implements OnItemSelectedListener {
		...
		public void onItemSelected(AdapterView<?> arg0, View arg1,
			...
			lesId = lessonId;
			CommonTool.setCurLesson(SentenceTranslateView.this, lessonId,"sentenceLessonId");
	...
	private class TextOnClick implements OnClickListener{
		...
		public void onClick(View arg0) {
			...
			if(tv.getTag()!=null){
				...
				if(tv.getText().toString().equals(at.getChtext())){
					tv.setText(tv.getText().toString()+"\n"+at.getJptext());
				}else{
					tv.setText(at.getChtext());
				}
			}
